...
.nice-select.open-fromScript .list {
    opacity: 1;
    pointer-events: auto;
...
.nice-select.open-fromScript:after {
        ...
        <div class="container banner-center-container">
          <div class="row">
            <div class="col-lg-6 col-md-8">
              ...
              <!--<form class="banner-form" action="/brief/" id="bannerForm">
                <img src="{{ env('IMAGE_PATH') == '' ? asset('frontend/') : env('IMAGE_PATH') }}/assets/images/banner/pencil.png" alt="">
                <input type="text" name="service" placeholder="Enter Your Business Name">
                <button type="submit">
                  Let's Build
                  <span></span>
                  <span></span>
            ...
            <form action="{{ route('brief') }}" method="GET" class="banner-form d-block" id="bannerForm">
                @csrf
                <div class="d-flex align-items-center banner-selectArea position-relative"> 
                    ...
                    <select name="service" class="wide" id="bannerselect">
                        <option selected disabled hidden value="" id="typewriter-option">Need a Website That Stands Out?</option>
        ...
        <div class="tab-content web-portfolio-content">
            ...
            <div class="row rowGap2">
              ...
              <div class="col-lg-4 col-md-6" data-index="7">
                <div class="single-webPortfolio static_portfolio__home">
                  <div class="single-webPortfolio">
                  <div class="webPortfolio-imageArea">
                    <div class="main-imageArea">
                      <div class="backImage">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        submitBannerForm.addEventListener('click', function(event) {
                ...
                // Get the input value
                const companyName = document.getElementById('bannerselect').value;
                
                if (!companyName) {
                    const btn = jQuery(document).find("#bannerForm .nice-select")[0];
                    btn.classList.add('open-fromScript');
                    return
                }
                ...
                if (stepForm && companyName) {
                    window.location.href = window.Laravel.routes.webBrief + `?service=${companyName}`;
                }
            });
        });

        function handleClick(event) {
        const niceSelect = document.querySelector("#bannerForm .nice-select");
        const submitButton = document.querySelector("#submitBannerForm");
    
        // Check if the click is on the submit button or one of its children
        ...
        // Check if the click is inside or outside the nice-select dropdown
        if (niceSelect && !niceSelect.contains(event.target)) {
            niceSelect.classList.remove("open-fromScript"); // Remove class on outside click
        } else if (niceSelect && niceSelect.contains(event.target)) {
            niceSelect.classList.remove("open-fromScript"); // Remove class on inside click
        }
    }
