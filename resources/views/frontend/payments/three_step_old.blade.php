...
<body class="bg-light generalClass">
            ...
            <div class="row">
                ...
                @if (!empty($brand_settings['company_phone']))
                    <div class="col-md-9 col-5 d-none d-sm-none d-md-block">
                        <a href="tel:{{ $brand_settings['company_phone'] }}" class="company_number"><i class="fa fa-phone"></i> <span>{{ $brand_settings['company_phone'] }}</span></a>
                    </div>
                @endif
            </div>
        </div>
    ...
    <div class="container padd-30-on-mob">
        ...
        <div class="row">
            ...
            <div class="col-md-8 order-md-1" id="main-formarea">
                ...
                <form id="payment-form" class="needs-validation" data-secret="{{ (!empty($intent) ? $intent->client_secret : "") }}">
                    @csrf
                    <div class="row">
                    ...
                    <div class="row">
                        ...
                        <div class="col-lg-7 col-md-12">
                            <input type="text" class="form-control cardNo" name="cardNo" maxlength="19" id="cardNo" placeholder="Card Number">
                            <span class="validation-error text-danger d-none"></span>
                            <div class="invalid-feedback">
                        ...
                        <div class="col-lg-5 col-md-12 d-flex">
                            <div class="col-lg-4 col-md-4">
                                <input type="text" maxlength="2" class="form-control month" id="month" name="month" placeholder="MM">
                                <span class="validation-error text-danger d-none"></span>
                                <div class="invalid-feedback">
                            ...
                            <div class="col-lg-4 col-md-4">
                                <input type="text" maxlength="4" class="form-control year" id="year" name="year" placeholder="YYYY">
                                <span class="validation-error text-danger d-none"></span>
                                <div class="invalid-feedback">
                            ...
                            <div class="col-lg-4 col-md-4">
                                <input type="text" maxlength="4" class="form-control cvc" id="cvc" name="cvc" placeholder="CVC">
                                <span class="validation-error text-danger d-none"></span>
                                <div class="invalid-feedback">
                                    CVC is required.
                                </div>
                    ...
                    <button class="btn btn-primary btn-lg btn-block" id="card-button"  type="submit">Pay Now</button>
    ...
    <script>
        ...
        $(document).on("submit", "#payment-form", function(e) {
            ...
            var clientSecret = payment_form.dataset.secret;
            ...
            axios({
                url: '{{ route("payment.stripe.createThreeStepPaymentMethod") }}',
                method: 'POST',
                headers: {
                ...
                payment_form.dataset.secret = clientSecret;
            ...
            function handleServerResponse(response) {
                ...
                if(response.data?.success || response.data?.status === "succeeded"){
                    ...
                    paymentData = $('#payment-form').serializeArray();
                    axios({
                        url: '{{ route("payment.stripe.PaymentIntent_succeeded") }}',
                        method: 'POST',
                        headers: {
                            'Accept': 'application/json',
                            'Content-Type': 'application/json;charset=UTF-8',
                        ...
                        data: { remainingPay: 'yes', payment_intent_id: response.data.id, payment_method_id: response.data.payment_method,customer_id: customer_id, paymentData:paymentData }
                    }).then(function (response) { 
                        console.log(response);
                        debugger;
                ...
                } else if (response.data?.requires_action || response.data?.status === "requires_action") {
                    ...
                    stripe.handleCardAction(
                        ...
                        if (result.error) {
                            ...
                            //var form_data = $("#payment-form").serialize();
                             ...
                             paymentData.push({name: "payment_method_id", value: result.paymentIntent.payment_method});
                            
                            // The card action has been handled
                            // The PaymentIntent can be confirmed again on the server
                            axios({
                            url: '{{ route("payment.stripe.paymentIntentThreeStep_3d") }}',
                            method: 'POST',
                            headers: {
                                'Accept': 'application/json',
                                'Content-Type': 'application/json;charset=UTF-8',
                            ...
                            if (response.data.error && response.data?.code != "authentication_required") {
                            console.log('error4');
                            //var form_values = getFormData('#payment-form');
                            // paymentMessages.push({name: "error_code", value: response.code});
                            // paymentMessages.push({name: "error_message", value: response.error});
